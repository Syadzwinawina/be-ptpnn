...
#     val = (user_id, password, nama, nik, jabatan)
...
#         else:
#              result = {
#                 "error": "Tinggi tidak ditemukan di tangki yang dipilih"
#             }
...
#         else:
#              result = {
#                 "error": "suhu tidak ditemukan di tangki yang dipilih"
#             }
...
#             # Menampilkan hasil beda
#             # print("Hasil beda:", beda)
...
#             else:
#                 print("Tidak dapat menghitung hasil Sounding karena nilai suhu tidak ditemukan.")
#         else:
#             print("Tidak dapat menghitung hasil Sounding karena nilai tangki tidak ditemukan.")

#         # Menutup koneksi database
#         cursor.close()
#         cnx.close()

#         return jsonify({"message": "Data Sounding telah dihitung"})
...
#             rata2_sounding = total_sounding / hasil_sounding
#             akurasi = (hasil_sounding - rata2_sounding) / rata2_sounding * 100
...
#             # Menghitung hasil Rendemen
#             hasil_rendemen = (hasil_cpo / TBS_olah) * 100
#             print("Hasil Rendemen:", hasil_rendemen)
            
#             # Menyimpan riwayat rendemen ke dalam database
#             waktu = datetime.now()
...
#         # sql_check_rendemen = ("SELECT hasil_rendemen FROM hasil_r WHERE tanggal = %s", tanggal )
#         sql_check_rendemen = "SELECT hasil_rendemen FROM hasil_r WHERE DATE(waktu) = %s"
#         val_check_rendemen = (tanggal,)
#         cursor.execute(sql_check_rendemen, val_check_rendemen)
#         result_rendemen = cursor.fetchall()
...
#                 # Menghitung hasil rendemen
#                 hasil_cpo = max(0, akurasi - pengiriman - stok_awal)
#                 print("Hasil CPO:", hasil_cpo)
...
#         val_check_rendemen = (tanggal,)
#         cursor.execute(sql_check_rendemen, val_check_rendemen)
#         result_rendemen = cursor.fetchone()

#         if result_rendemen and result_rendemen[0] > 0:
#             print("Rendemen telah dilakukan pada tanggal tersebut.")
#             return jsonify({
#                 "error": "true",
#                 "message": "Rendemen telah dilakukan pada tanggal tersebut"
...
#                 if hasil_rendemen <= 0:
#                     print("Hasil rendemen tidak valid.")
#                     return jsonify({
#                         "error": "true",
#                         "message": "Hasil rendemen tidak valid"
#                     })
...
# @app.route('/hapus_pengguna', methods=['POST'])
# def hapus_pengguna():
#     try:
#         data = request.get_json()
#         if 'user_id' in data:
...
# @app.route('/download_hasil_sounding', methods=['POST'])
...
#         query = "SELECT waktu, nama, tangki, temperatur_suhu, tinggi, volume, hasil_sounding FROM hasil_s WHERE DATE(waktu) = %s"
#         cursor.execute(query, (tanggal_input,))

#         # Mendapatkan hasil query
#         results = cursor.fetchall()

#         # Mengubah hasil query menjadi tabel
#         headers = ["Waktu", "Nama", "Tangki", "Temperatur Suhu", "Tinggi", "Volume", "Hasil Sounding"]
#         table_data = []
#         for result in results:
...
#         # Menyiapkan file PDF dengan format landscape
...
#         return jsonify({"message": f"File PDF '{filename}' berhasil dihasilkan."})
...
#     tangki_data = [result[3] for result in results]
#     suhu_data = [result[4] for result in results]
#     tinggi_data = [result[5] for result in results]
#     volume_data = [result[6] for result in results]
#     beda_data = [result[7] for result in results]
#     sounding_data = [result[8] for result in results]
...
#         "nama": nama_data,
#         "tangki": tangki_data,
#         "temperatur_suhu": suhu_data,
#         "tinggi": tinggi_data,
#         "volume": volume_data,
#         "beda": beda_data,
#         "hasil_sounding": sounding_data
#     })



# @app.route('/download_hasil_rendemen', methods=['POST'])
...
#         table_data = []
#         for result in results:
#             table_data.append([result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7], result[8]])
...
#     # Menjalankan query untuk mengambil data hasil berdasarkan bulan
#     query = "SELECT * FROM hasil_s WHERE MONTH(Waktu) = %s"
...
#     # Mengubah hasil query menjadi tabel dengan menggunakan tabulate
#     headers = ["ID", "Waktu", "Nama", "Tangki", "Temperatur_Suhu", "Tinggi", "Volume", "Beda", "Hasil_Sounding"]
#     table_data = []
#     for result in results:
#         table_data.append([result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7], result[8]])  # Sesuaikan dengan kolom yang ada di tabel
...
#     # Menyiapkan file PDF dengan format landscape
#     filename = f"hasil_sounding_{bulan}.pdf"
#     doc = SimpleDocTemplate(filename, pagesize=landscape(letter))
...
# @app.route('/hasil_sounding_admin', methods=['POST'])
# def hasil_sounding_admin():
#     data = request.get_json()
#     bulan = data.get('bulan')

#     if bulan is not None:
#         filename = generate_pdf(bulan)
...
#     # Menyiapkan file PDF dengan format landscape
#     filename_r = f"hasil_rendemen_{bulan}.pdf"
#     doc = SimpleDocTemplate(filename_r, pagesize=landscape(letter))
...
# @app.route('/hasil_rendemen_admin', methods=['POST'])
# def hasil_rendemen_admin():
#     data = request.get_json()
#     bulan = data.get('bulan')

#     if bulan is not None:
#         filename_r = generate_rendemen(bulan)
#         return jsonify({'filename_r': filename_r})
#     else:
#         return jsonify({'error': 'Invalid request'})
...
# @app.route('/hapus-data/<int:user_id>', methods=['DELETE'])
# def hapus_data(user_id):
#     try:
#         # Membuat kursor database
